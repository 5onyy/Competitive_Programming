#include<bits/stdc++.h>
#include <ext/pb_ds/assoc_container.hpp>
#include <ext/pb_ds/tree_policy.hpp>

using namespace __gnu_pbds;
using namespace std;

#define int long long
#define f first
#define s second
#define pb push_back
#define sz(x) (int)(x).size()
#define sqr(x) ((x) * (x))
#define all(x)         x.begin(),x.end()
#define rr(x)          sort(all(x)),x.resize((unique(all(x))-x.begin()));
#define rep(i, a, b) for (int i = a; i < (b); ++i)
#define timer cerr << "Time elapsed: " << 1.0 * clock() / CLOCKS_PER_SEC << " s.\n";

template<typename T> using vt = vector<T>;
using ll = long long;
using ld = long double;
using vi = vt<int>;
using ii = pair<int, int>;
using iii = pair<int, ii>;
using vii = vt<ii>;

const ll INF = 2e18 + 5;
const int inf = 2e9 + 5;
const int mod = 1e9 + 7;
const int td[8] = { -1, 0, 1, 0, -1, -1, 1, 1}, tc[8] = {0, 1, 0, -1, -1, 1, 1, -1};
template<typename T> using minpq = priority_queue<T, vt<T>, greater<T>>;
template<typename T> using maxpq = priority_queue<T>;
template<class T>	using ordered_set = tree<T, null_type, less<T>, rb_tree_tag, tree_order_statistics_node_update>;

ll fpow(ll a, ll b, const ll c) { ll ans = 1; a %= c; for (; b; b >>= 1, a = a * a % c) if (b & 1) ans = ans * a % c; return ans;}
ll fpow(ll a, ll b) {ll ans = 1; for (; b; b >>= 1, a *= a) if (b & 1) ans *= a; return ans;}

const int N = 101;
int n, m, k;    

struct Matrix {
    int mat[N][N];
    Matrix() {memset(mat, 0, sizeof mat);}
    Matrix I() {
        Matrix ans;
        for (int i = 0; i < n; i++)    ans.mat[i][i] = 1;
        return ans;
    }
};

Matrix matMul(Matrix a, Matrix b) { // O(N^3)
    Matrix ans;
    for (int i = 0; i < n; i++) for (int j = 0; j < n; j++) ans.mat[i][j] = INF;
    for (int i = 0; i < n; i++)     for (int j = 0; j < n; j++)
    for (int k = 0; k < n; k++) {
        if (a.mat[i][k] == INF || b.mat[k][j] == INF)   continue;
        ans.mat[i][j] = min(ans.mat[i][j], a.mat[i][k] + b.mat[k][j]);
    }
    return ans;
}

Matrix epow(Matrix a, int n) {
    Matrix ans;
    if (n == 0) return ans.I();
    if (n == 1) return a;
    if (n % 2 == 0)     {
        Matrix t = epow(a, n / 2);
        return matMul(t, t);
    } 
    else {
        Matrix t = epow(a, n / 2);
        return matMul(matMul(t,t),a);
    }
}

void solve() {
    cin >> n >> m >>k;
    cout << INF << '\n'; 
    cout << (INF > (1000000000LL * 1000000000LL)) << '\n';
    Matrix state;
    for (int i = 0; i < n; i++)     for (int j = 0; j < n; j++) state.mat[i][j] = INF;
    for (int i = 0; i < m; i++) {
        int u,v,w;    cin >> u >> v >> w;  u--, v--;
        state.mat[u][v] = w;
    }
    Matrix ans = epow(state, k);
    int res = INF;
    for (int i = 0; i < n; i++){    
        for (int j = 0; j < n; j++)        {//cout << ans.mat[i][j] << ' ';
        res = min(res, ans.mat[i][j]);
        }
        //cout << '\n';
    }
    if (res == INF)    cout << "IMPOSSIBLE";
    else cout << res;
}

int32_t main() {
    ios_base :: sync_with_stdio(0);  cin.tie(nullptr);   cout.tie(nullptr);
    // freopen("t.inp", "r", stdin);
    // freopen("t.out", "w", stdout);
    solve();
    return 0;
}
